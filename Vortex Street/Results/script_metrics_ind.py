...
# metrics_pareto_ = []
# for i in range(len(metrics_pareto)):
#     metrics_pareto_.append(np.asarray(metrics_pareto[i])[0])

# metrics_pareto = metrics_pareto_
# print("metrics_pareto:", metrics_pareto)
# #print(metrics_pareto.shape)
...
#     x.append(np.asarray(x_[i])[0])
#     y.append(np.asarray(y_[i])[0])
...
annot = ax.annotate("", xy=(0,0), 
    xytext=(0,5), 
    ha='center',
    textcoords="offset points", bbox=dict(boxstyle="round", fc="w")) # arrowprops=dict(arrowstyle="->"))
...
plt.suptitle("Vortex Street, Pareto frontier", fontsize=20)
...
plt.savefig('cfd_pareto_ind.png', dpi=300)
plt.show()
