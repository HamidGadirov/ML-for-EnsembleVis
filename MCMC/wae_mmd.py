...
file = open('mcmc_labelled_data.pkl', 'rb')
X_train = pickle.load(file)
file.close()

file = open('mcmc.pkl', 'rb')
Y_train = pickle.load(file)
file.close()

trainset = YourDataset(X_train, Y_train, transform=transforms.Compose([transforms.ToTensor()]))
print(len(trainset))

testset = YourDataset(Y_train, X_train, transform=transforms.Compose([transforms.ToTensor()]))
print(len(testset))
...
for epoch in range(args.epochs):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        reconst = decoder(z_real).cpu().view(batch_size, 1, 50, 50)
        sample = decoder(torch.randn_like(z_real)).cpu().view(batch_size, 1, 50, 50)

        if not os.path.isdir('./data/reconst_images'):
            os.makedirs('data/reconst_images')

        save_image(test_data[0].view(-1, 1, 50, 50), './data/reconst_images/wae_mmd_input.png')
